...
public class KeHuDao {
    ...
    private KeHuDBHelper dbHelper;
    private Cursor cursor;

    public KeHuDao(KeHuDBHelper dbHelper) {
        this.dbHelper = dbHelper;
    }

    // 添加一个Student对象数据到数据库表
    public long addStudent(KeHu s) {
    ...
    public int updateStudent(KeHu s) {
        ContentValues values = new ContentValues();
        values.put(TableContanst.StudentColumns.NAME, s.getName());
        values.put(TableContanst.StudentColumns.AGE, s.getAge());
        values.put(TableContanst.StudentColumns.SEX, s.getSex());
    ...
    public KeHu getStudentFromView(View view, long id) {
        ...
        KeHu student = new KeHu(id, name, age, sex, like, phone, data, null);
        return student;
    }
